...
const Footer = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/sponsors/`);
    ...
    <div>    
        ...
        <footer className="footer">
            <div className="container">
                <h3 className='akira text-center my-4'>our sponsors</h3>
                <div className="row center-image my-5">
                    {sponsors.map((sponsor, index) => (
                        <div className="col-4" key={index}>
                            <div className="center-image">
                                <img src={sponsor.image} className="img-fluid" alt={sponsor.brand} />
                            </div>
                        </div>
                    ))}
                </div>
